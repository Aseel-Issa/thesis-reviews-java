			...
			try {
				...
				do{
			        ...
			        if (pageCount == 0) {
			        	...
			        	myWaitVar.until(ExpectedConditions.elementToBeClickable(By.id("filters_button")));
...
//			        	out.println(chunk.toString());
			        ...
			        try{
			        	WebElement next = driver.findElement(By.className("pagination__next"));
						...
						js.executeScript("document.querySelector('div.pagination__next > span > a').click();");
						// add , after each json object "chunk" except for the last one and write it into the json file
				        if(pageCount!=stopPoint)
				        	out.println(chunk.toString()+","); //.toString()
				        else
